...
| `status/bot/heartbeat` | `Bot` | QoS 0, not retained. Exact versioned liveness, Discord fields, and dependency flags in `contracts/telemetry.md` §2.1. |
| `status/ai_worker/heartbeat` | `AI Worker` | QoS 0, not retained. Exact versioned run/pause state, active-task count, and RabbitMQ flag in `contracts/telemetry.md` §2.2. |
...
| `status/bot/heartbeat`, `status/ai_worker/heartbeat` | `Head` | Every 30 seconds by default; stale after 90 seconds without a valid receipt (`contracts/telemetry.md` §2.3) |
...
- **Heartbeat validation/staleness (resolved, P1.8):** Bot and AI Worker schemas are canonical in `contracts/telemetry.md` §2. Invalid/unknown-version payloads do not refresh liveness. Head marks a service stale after `HEAD_SERVICE_HEARTBEAT_STALE_SEC` (default 90) using local monotonic receipt time; payload wall-clock timestamps are audit-only.
