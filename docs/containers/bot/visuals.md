...
| `LobbyView` | `JoinButton`/`LeaveButton`/`StartButton`/`AbortButton`/`CreatorAbortButton` + the 1-second tick timer | Live participant list + countdown + role-gated action buttons (only the lobby creator sees Start/Abort) | `bot/modules/UI/views/lobby.py` | `commands/quick-battle.md` (drafted) |
...
- **`Duration`** — elapsed wall-clock time since `TaskRecord.created_at` (`discord_bot.md` §6.3 — the moment `Bot` published the `ai_tasks` message, not the first `progress/ai_worker` tick received), formatted as `Xm Ys`. Recomputed on every re-render, not just on a phase change, since a slow-running `refining` loop with no new ticks for a while should still visibly show elapsed time increasing — meaning this container needs its own lightweight periodic re-render tick (e.g. every 5–10s) independent of Mosquitto tick arrival, not purely event-driven. Exact re-render interval isn't pinned down further than "a few seconds" — an implementation detail, not a design decision.
...
- **`WizardView`'s original description didn't match reality** (§3) — drafting `commands/suggest.md` revealed the legacy `SuggestionTypeSelect`/`SuggestionCategorySelect`/`SuggestionModal` chain is a single simultaneous-selects-plus-modal-trigger view, not a sequential multi-step wizard. Corrected in §3's table (renamed `SuggestionView`); flagged here since this is the kind of "later doc corrects an earlier assumption" pattern `Readme.md`'s own notes already track for other files.
- **`/suggest`'s category/type list is moving off a hardcoded Python list onto `azure.md`'s `status.py` shared document** (project owner, `commands/suggest.md` §6/§9) — this doesn't change anything in this file directly (no `visuals.md` component is involved), but is recorded here since it's a `/suggest`-adjacent architecture decision made in the same pass as the `WizardView` correction above. See `commands/suggest.md` §14 and `web/pages/suggestions.md` for the full thread — `azure.md` itself is deliberately left untouched by this decision for now (project owner).
