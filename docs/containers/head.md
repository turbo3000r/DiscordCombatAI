...
| `HEAD_METRICS_BUFFER_MAX_BATCHES` | No | `60` | Maximum completed metrics windows queued in memory after Table failures (one hour at the default batch cadence). Drop oldest on overflow; canonical policy: `contracts/telemetry.md` §3. |
...
| `HEAD_TELEMETRY_LIVE_MAX_LOGS` | No | `50` | Maximum newest log lines considered per live tick (`contracts/telemetry.md` §5). |
| `HEAD_TELEMETRY_LIVE_MAX_BYTES` | No | `65536` | Hard cap on the final UTF-8 JSON `telemetry_live` payload; oldest selected logs are omitted first (`contracts/telemetry.md` §5). |
| `HEAD_SERVICE_HEARTBEAT_STALE_SEC` | No | `90` | Bot/AI Worker heartbeat staleness threshold, measured from local monotonic receipt time. Must be at least twice the largest configured service-heartbeat interval (`contracts/telemetry.md` §2.3). |
...
| `Bot` | Mosquitto, `status/bot/heartbeat` | Versioned liveness, Discord fields, and bounded dependency flags (`contracts/telemetry.md` §2.1) | Every `BOT_HEARTBEAT_INTERVAL_SEC` |
| `AI Worker` | Mosquitto, `status/ai_worker/heartbeat` | Versioned liveness, run/pause state, active task count, and RabbitMQ status (`contracts/telemetry.md` §2.2) | Every `AI_WORKER_HEARTBEAT_INTERVAL_SEC` |
...
| GitHub Releases API | HTTPS, polled | JSON release metadata; drafts ignored, prereleases ignored unless explicitly enabled | Every `HEAD_RELEASE_POLL_INTERVAL_SEC` |
...
| Azure Web PubSub | Service SDK `send_to_group` | Live `telemetry_live` payload (`contracts/telemetry.md` §5) to `HEAD_PUBSUB_DASHBOARD_GROUP` | Every `HEAD_TELEMETRY_LIVE_INTERVAL_SEC` **while leader** — always-stream, no listener detection (`contracts/pubsub_live.md`) |
| Azure Table Storage | HTTPS | Batched metrics rows (`contracts/telemetry.md` §3) | Every `HEAD_TELEMETRY_BATCH_INTERVAL_SEC` — **leader only** |
...
- **Failure transitions:** PubSub-only failure, Blob lease-renew-only failure, and Mosquitto failure cause bounded soft-stop. A Blob-renew-only failure may recover only by confirming the **same lease term** within the drain bound and issuing a fresh same-term grant; it never creates a new term in place. Simultaneous loss of both Azure coordination dependencies causes immediate hard-stop. If this Head learns another Head owns leadership, it immediately commands hard-stop and ceases grants. Head crash needs no final publish: Bot hard-stops autonomously when its live grant/watchdog expires. Full ordering and recovery conditions are canonical in `contracts/leadership_control.md` §5.
...
`Head` aggregates structured logs from the **local node's** Mosquitto broker. Cross-node log routing is not part of v1.
...
- **Service heartbeat freshness:** Bot and AI Worker use the exact schemas in `contracts/telemetry.md` §2. After `HEAD_SERVICE_HEARTBEAT_STALE_SEC` without a valid message, Head marks that service stale; stale Bot latency/guild fields become `null`. This is diagnostic, not a leadership transition.
...
- **Batched persistence:** every `HEAD_TELEMETRY_BATCH_INTERVAL_SEC` → Table Storage — **leader only**. Failed completed windows queue in a 60-entry FIFO by default; recovery writes oldest-first and overflow drops the oldest window (`contracts/telemetry.md` §3). Followers do not upload dashboard telemetry.
...
- **Live cap:** at most 50 newest logs and 65,536 serialized UTF-8 bytes per tick; omit oldest selected logs first and report `logs_dropped` (`contracts/telemetry.md` §5).
...
| Mosquitto unreachable | Publish/subscribe failures | No activation is permitted. An already active Bot detects control loss and soft-stops, then hard-stops by drain/grant deadline. **On reconnect (P1.5):** re-publish current retained `control/bot/desired_state` and `control/ai_worker/desired_state` from in-memory authority; do **not** replay a backlog of grants (`mosquitto.md` §6). |
...
| Azure Blob Lease renewal fails while PubSub remains available | Exception from `lease_client.py` | Soft-stop immediately; bounded retries may restore only the same lease term. Issue a fresh same-term active grant only after authority is confirmed; otherwise hard-stop at the drain deadline. |
...
| Azure Table Storage | Metrics persistence | Failed completed windows queue in memory up to `HEAD_METRICS_BUFFER_MAX_BATCHES` (default 60), then the oldest window is dropped. Recovery uploads oldest-first (`contracts/telemetry.md` §3). |
...
It may return `503` with `status: "initializing"` until initialized. `version` is the required Compose-injected `APPLICATION_VERSION`. The endpoint remains liveness-only: it says nothing about leadership, Discord connectivity, Bot readiness, or dependency readiness. Launcher verification succeeds only when authentication and payload validation pass, `status == "alive"`, **and `version` exactly equals the admitted target version**. HMAC headers, timeout/retry behavior, and the exact schema are canonical in `contracts/launcher_ipc.md` §3/§5.
...
- Automatic polling accepts tags only under the exact grammar in `contracts/launcher_ipc.md` §4, ignores GitHub drafts, and ignores prereleases unless `HEAD_RELEASE_INCLUDE_PRERELEASE=true`. It compares parsed SemVer precedence against required `APPLICATION_VERSION`; it never compares tags lexically.
