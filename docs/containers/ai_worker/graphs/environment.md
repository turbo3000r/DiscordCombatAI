...
There is also a **third, deliberately out-of-scope mode**: if players choose not to describe a custom environment at all, the caller picks a pre-written arena from `prompts/static/generic_environments/*.txt` (target path; legacy on-disk may still be `prompts/core/generic_environments/` until migration — see `ai_worker/prompts.md` §6) directly and **never invokes this graph** — no LLM call, no state, nothing to validate. This mirrors the legacy `custom_environment` boolean toggle (`docs/legacy/Old_arch.md` line 216). Confirmed decision: `environment` is only ever invoked for `initial` or `revision`. Which container image owns the generic-arena files is still **P1.1 item 11**.

> **Confirmed working assumption (project owner):** `environment` is invoked as its own distinct `ai_task` during the lobby phase, before fighters/strategies are collected — `battle` receives the finished `Environment` the same way it receives `setting` (as part of its own input), not as a graph it's ever nested inside. Lobby sequencing/handoff mechanics are owned by `bot/commands/quick-battle.md` (drafted) and remaining **P1.1** product rules — there is no `ai_worker/graphs/quick-battle.md`.
...
- The exact task-boundary mechanics between `environment` and `battle` (one `ai_task` per phase? how the lobby sequences them?) remain under **P1.1** / `bot/commands/quick-battle.md` — not an unwritten `ai_worker/graphs/quick-battle.md`. "Are they the same graph" is settled (§1: no).
