...
- **Phase 0 doc prerequisites (P1.5, P1.7, P1.9, and the P1.3 Azure-client subset) are resolved** — see `to_resolve.md` Resolved decisions. Brokers, shared Azure clients, locale enum/mapping, and guild Patch/soft-delete helpers are implementation-ready.
- **Phase 3 documentation gate is closed** — ProcessCommand, `/config`, `/suggest`, Queue/sweep/DM delivery, minimal Web Suggestions slice, and S12 are specified in `to_resolve.md` → Phase 3. Still deferred from Phase 3 implementation: offline guild-removal sweep, `/quick-battle`, non-Suggestions Web pages, webhook/S13; P1.1/P1.2 documentation is now closed separately below.
...
- **Phase 4/5 documentation gates are closed.** P1.1 Quick Battle session/Discord behavior and P1.2 graph schemas/rubrics/bounds are canonical in `quick-battle.md`, `ai_worker/nodes.md`, both graph docs, shared task contracts, and concrete S11. Implementation is split into Phase 4A shared contracts, 4B environment, 4C battle, then Phase 5 command orchestration (`to_resolve.md`).
