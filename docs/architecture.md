...
**Application version injection:** Compose injects `APPLICATION_VERSION` into `head`, `bot`, and `ai_worker`. Image tags use `${APPLICATION_VERSION:-local}` so an unset host variable still runs the locally built image. Process env uses `${APPLICATION_VERSION:-v0.1.0}` (same tag as `.env.example`, grammar in `contracts/launcher_ipc.md` §4) so an unset host variable is still a valid release tag — an explicit empty string remains invalid and the process fails startup. During Launcher operations, the coordinator supplies the admitted target as the child Compose process's `APPLICATION_VERSION`, which also selects the coordinated image tags. Direct/manual Compose may set it explicitly. Each application process fails startup if the value is absent or does not match `contracts/launcher_ipc.md` §4.
