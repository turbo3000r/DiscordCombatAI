...
- **Discord rate limits:** node transitions and same-phase heartbeat ticks never directly produce Discord edits. Bot renders a phase transition through the bounded scheduler in `bot/discord_bot.md` §6.3 and refreshes elapsed duration at most every 15 seconds. This keeps broker liveness cadence independent from Discord write cadence.
...
- **`AI_WORKER_PROGRESS_HEARTBEAT_SEC=30` is confirmed** with the 120-second stall timeout and 600s/840s graph deadlines (`contracts/ai_task.md` §5/§12).
