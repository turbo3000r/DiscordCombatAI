...
| **Not automatic** | Not sourced from Discord `interaction.locale` for guild commands. Default for never-configured guilds: `"en"` (`contracts/guild_config.md` §5). Production DM `/suggest` denials/UI may use interaction/user locale with the §3b fallback chain (`discord_bot.md` §6.4). |
