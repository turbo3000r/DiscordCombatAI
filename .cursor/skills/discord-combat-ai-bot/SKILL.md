...
2. Read `docs/containers/bot/discord_bot.md` (especially §6.0 concurrency and Phase 2 scope).
3. Read owning contracts for the slice: `leadership_control`, `ai_task`, `task_progress`, `drain_status`, `telemetry`, `guild_config`, and command docs only when the slice includes them.
4. Read only the scenarios the slice claims (typically S03, S04, S05, S07, S08, S10).
5. Check `docs/to_resolve.md` for Phase 2 gates and remaining P1 items that defer commands/graphs.
...
Do not invent slash commands, `ProcessCommand` behavior, lobby persistence, or Discord interaction recovery when those decisions are deferred. Stop and report the missing decision.
...
- full `ProcessCommand` abstraction (deferred to `/config`, `/suggest`, `/quick-battle`);
- suggestion queue poller/sweep;
- lobby/collector/vote workflow units;
- claiming Discord interaction recovery for orphaned tasks (v1 limitation preserved).

### Later phases

- Phase 3: `/config`, `/suggest` (+ `ProcessCommand` as needed).
...
- at-least-once delivery with effectively-once outcome by `task_id`;
- no temporary Discord stub command for Phase 2 transport proof;
...
- Do not widen Azure roles or log `DISCORD_BOT_TOKEN` / guild `api_key`.
- Do not claim exactly-once delivery or invent durable orphan-task reconciliation beyond contracts.
- Do not register user-facing slash commands in Phase 2 slices.
...
- concurrency: broker/MQTT threads do not mutate state without loop handoff.

Mark scenario steps `complete`, `integration-only`, or `deferred` per `docs/scenarios/` Phase 2 ownership matrices. Do not claim full S03–S10 acceptance when deferred Discord command surfaces remain.

A slice is complete only when:

- applicable Phase 2 gates are closed or explicitly out of scope;
- focused tests and static checks pass;
- secrets remain redacted;
- handoff names the next Bot, AI Worker, or command-phase slice.
