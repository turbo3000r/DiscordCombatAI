...
def test_entrypoint_scripts_use_uv_no_sync() -> None:
    ...
    assert "uv run --no-sync uvicorn" in web


def test_dockerfiles_copy_entrypoint_outside_src_bind_mount() -> None:
    """Dev overlay mounts ./src:/app/src. Windows CRLF shebangs must not be exec'd."""
    for relative in (
        "src/bot/Dockerfile",
        "src/ai_worker/Dockerfile",
        "src/dev_support/Dockerfile",
        "src/web/Dockerfile",
    ):
        content = (ROOT / relative).read_text(encoding="utf-8")
        assert "cp /app/src/" in content
        assert "/app/entrypoint.sh" in content
        assert 'ENTRYPOINT ["/app/src/' not in content


def test_entrypoint_scripts_are_unix_lf() -> None:
    for relative in (
        "src/bot/entrypoint.sh",
        "src/ai_worker/entrypoint.sh",
        "src/dev_support/entrypoint.sh",
        "src/web/entrypoint.sh",
    ):
        data = (ROOT / relative).read_bytes()
        assert b"\r" not in data, f"{relative} must use LF line endings"
